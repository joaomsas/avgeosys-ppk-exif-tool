...
def test_full_pipeline(tmp_path, setup_full, monkeypatch):
    ...
    def fake_process_single_folder(
        folder, base_obs, base_nav, use_rover_nav=True
    ):
    ...
    monkeypatch.setattr(
        ...
        fake_process_single_folder,
    )
    monkeypatch.setattr(
        ppk_mod,
        "find_base_files",
        lambda r: (Path("b"), Path("c")),
    )
    ...
    monkeypatch.setattr(
        ...
        lambda mrk, od: pd.DataFrame(
            {"index": [1], "time": [0], "lat": [0], "lon": [0], "height": [0]}
        ),
    )
    monkeypatch.setattr(
        interp_mod,
        "load_pos_data",
        lambda pos: pd.DataFrame(
            {
                "seconds": [0],
                "lat": [0],
                "lon": [0],
    ...
    monkeypatch.setattr(
        exif_mod,
        "update_exif",
        lambda photo_path, lat, lon, height: None,
    )
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["avgeosys", str(root), "--all"],
    )
    cli_main()

    # 5) Verifica artefatos finais
    assert (
        root / "relatorio_processamento.txt"
    ).exists(), "Relatório não gerado"
    assert (
        root / "resultado_interpolado.kmz"
    ).exists(), "KMZ de interpolação não gerado"
    assert (
        root / "compilado_exif_data.kmz"
    ).exists(), "KMZ de EXIF não gerado"
