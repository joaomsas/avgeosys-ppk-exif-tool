...
from avgeosys.core.ppk import find_base_files, process_all_folders, process_single_folder
...
def cmd_interpolate(path: Path):
    ...
    for rd in path.rglob("PPK_Results"):
        ...
        if not mrk or not pos: continue
        mrk_df = preprocess_and_read_mrk(mrk, rd)
        pos_df = load_pos_data(pos)
        if mrk_df.empty or pos_df.empty: continue
...
def main():
    p = argparse.ArgumentParser(prog="avgeosys", description="AVGeoSys - PPK & EXIF Tool")
    ...
    p.add_argument("--interpolate", action="store_true", help="Interpolação de posições")
    ...
    if args.all or args.ppk:         cmd_ppk(args.path)
    if args.all or args.interpolate: cmd_interpolate(args.path)
    if args.all or args.geotag:      cmd_geotag(args.path)
    if args.all or args.report:      cmd_report(args.path)
