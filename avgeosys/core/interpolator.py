...
def load_pos_data(pos_file: Path) -> pd.DataFrame:
    """
    Lê o arquivo .pos, ignora comentários e retorna DataFrame com colunas:
    week, seconds, lat, lon, height, quality.
    """
    try:
        df = pd.read_csv(
            pos_file,
...
def interpolate_positions(pos_data: pd.DataFrame, mrk_data: pd.DataFrame) -> List[Dict]:
    ...
    if pos_data.empty or mrk_data.empty:
        logging.warning("Dados insuficientes para interpolação.")
        return []
